...
public:
     ...
     * @param new_value is the new value of index_block.*/
     ...
     int internal_block_space () const {
...
public:
    ...
    void listAll() {
        ...
        for (i = 0; i < DISK_SIZE; i++)
        {
            ...
            assert(ret_val >= 0);
            if (BitVector[i / block_size] == 2) {
                for (itr = MainDir.begin() ; itr != MainDir.end() ; itr++ ) {
                    if(itr->second->getFileInfo()->get_index_block() == i / block_size)
                        break;
                }
                ...
                if (file_size != 0 && i%block_size <= (file_size-1)/block_size)
                    cout << (int) bufy;
                else
                    cout << bufy;
            }
    ...
    int CreateFile(string fileName) {
        ...
        file_info->set_index_block(empty_block);

        /*set the block to be taken I've taken advantage of that the Bit Vector is from int type ,so I've defined 2 to
         * present block that taken to be index_Block:*/
        BitVector[empty_block] = 2;
    ...
    int WriteToFile(int fd, char *buf, int len ) {
         ...
         * 5. what need to return if success?
         * */
        /*checking if the disk pass format:*/
        if(!is_formated){
            fprintf(stderr , "The disk isn't formatted.\n");
        ...
        while (i < len) {
            ...
            if (OpenFileDescriptor[fd]->getFileInfo()->internal_block_space()) {
                ...
                location = (int) buffer * block_size;
                location += OpenFileDescriptor[fd]->getFileInfo()->getfile_size() % block_size;
                ...
                decToBinary(empty_block , binary_empty_block);
    ...
    * The reading could preform only if the disk have formatted and the file is exist and open.
    ...
    int ReadFromFile(int fd, char *buf, int len ) {
        ...
        // todo need to ask what value to return here:
        return 0;
    }
     ...
     * @return if the function failed the return value will be -1, other wise will be 0.*/
    int DelFile( string FileName ) {
        ...
        if(file_size != 0){
            for (int i = 0 ; i <=((file_size-1) / block_size) ; i++){
                ...
                assert(ret_val == 1);
                BitVector[(int)buf] = 0;
                int block_location = (int)buf * block_size;
        ...
        delete(itr->second);
        MainDir.erase(itr);
        // todo need to check the return value here:
        return 0;
    }
